...
function itemTypeAlias(item: ApiTypeAlias) {
	return {
		...itemInfo(item),
		typeParameters: itemTypeParameters(item),
		unionMembers: itemUnion(item).flatMap((member) =>
...
export async function generateSplitDocumentation({
	...
	for (const pkgName of PACKAGES) {
		...
		for (const version of versions) {
			...
			await writeSplitDocsToFileSystem({ member: members, tag: version, overrideName: 'sitemap' });
			...
			for (const member of members) {
				...
				await writeSplitDocsToFileSystem({ member: returnValue, tag: version });
			}
		}
	}
}
